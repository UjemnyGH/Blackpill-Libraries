...
#include <stdbool.h>
#include "../src/gpio.h"
#include "../additional_libs/spi_software.h"
...
#define ST7735_RES_Pin       3
#define ST7735_RES_GPIO_Port IO_B
#define ST7735_CS_Pin        4
#define ST7735_CS_GPIO_Port  IO_B
#define ST7735_DC_Pin        5
#define ST7735_DC_GPIO_Port  IO_B
...
static void ST7735_Select() {
    setOutputGPIO(ST7735_CS_GPIO_Port, ST7735_CS_Pin, 0);
}

void ST7735_Unselect() {
    setOutputGPIO(ST7735_CS_GPIO_Port, ST7735_CS_Pin, 1);
}

static void ST7735_Reset() {
    setOutputGPIO(ST7735_RES_GPIO_Port, ST7735_RES_Pin, 0);
    delay(20);
    setOutputGPIO(ST7735_RES_GPIO_Port, ST7735_RES_Pin, 1);
}

SPI_Software_TX spi;

static void ST7735_WriteCommand(uint8 cmd) {
    setOutputGPIO(ST7735_DC_GPIO_Port, ST7735_DC_Pin, 0);
    SPI_SW_TX_TransferByte(&spi, cmd);
    //HAL_SPI_Transmit(&ST7735_SPI_PORT, &cmd, sizeof(cmd), HAL_MAX_DELAY);
}

/*static void ST7735_WriteData(uint8* buff, uint16 buff_size) {
    setOutputGPIO(ST7735_DC_GPIO_Port, ST7735_DC_Pin, 1);
    SPI_SW_TX_TransferData(&spi, (SPI_Data){.data = *buff, buff_size});
    //HAL_SPI_Transmit(&ST7735_SPI_PORT, buff, buff_size, HAL_MAX_DELAY);
}*/

static void ST7735_ExecuteCommandList(const uint8 *addr) {
    uint8 numCommands, numArgs;
    uint16 ms;
    ...
    while(numCommands--) {
        ...
        numArgs &= ~DELAY;
        if(numArgs) {
            // ST7735_WriteData((uint8*)addr, numArgs);
            setOutputGPIO(ST7735_DC_GPIO_Port, ST7735_DC_Pin, 1);
            for(int i = 0; i < numArgs; i++)
                SPI_SW_TX_TransferByte(&spi, addr[i]);
...
static void ST7735_SetAddressWindow(uint8 x0, uint8 y0, uint8 x1, uint8 y1) {
    // column address set
    ST7735_WriteCommand(ST7735_CASET);
    uint8 data[] = { 0x00, x0 + ST7735_XSTART, 0x00, x1 + ST7735_XSTART };
    setOutputGPIO(ST7735_DC_GPIO_Port, ST7735_DC_Pin, 1);
    SPI_SW_TX_TransferByte(&spi, data[0]);
    SPI_SW_TX_TransferByte(&spi, data[1]);
    SPI_SW_TX_TransferByte(&spi, data[2]);
    SPI_SW_TX_TransferByte(&spi, data[3]);
...
void ST7735_DrawPixel(uint16 x, uint16 y, uint16 color) {
...
void ST7735_FillRectangle(uint16 x, uint16 y, uint16 w, uint16 h, uint16 color) {
    // clipping
...
*/
void ST7735_FillScreen(uint16 color) {
...
void ST7735_SetGamma(GammaDef gamma)
{
	...
	//ST7735_WriteData((uint8 *) &gamma, sizeof(gamma));
    setOutputGPIO(ST7735_DC_GPIO_Port, ST7735_DC_Pin, 1);
    SPI_SW_TX_TransferByte(&spi,(uint8)gamma);
	ST7735_Unselect();
}
